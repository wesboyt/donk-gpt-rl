...
# 2. Disable Tokenizers Parallelism to prevent deadlocks
...
class DistributionalOTUtils:
    def __init__(self, device, min_return_bb=-200, max_return_bb=500, n_atoms=101):
        self.device = device
    ...
    def to_categorical(self, rewards_list, big_blinds):
        ...
        for i, r in enumerate(rewards_list):
            ...
            m = torch.zeros(self.n_atoms, device=self.device)
            m.scatter_add_(0, l, (u.float() - b))
            m.scatter_add_(0, u, (b - l.float()))

            distributions[i] = m / (m.sum() + 1e-8)

        return distributions

    def compute_pairwise_wasserstein(self, dists):
        cdf = torch.cumsum(dists, dim=-1)
        cdf_a = cdf.unsqueeze(2)
        cdf_b = cdf.unsqueeze(1)
        diff = torch.abs(cdf_a - cdf_b)
        w1_matrix = torch.sum(diff, dim=-1) * self.delta_z
        return w1_matrix


class LogSinkhornLoss(torch.nn.Module):
    def __init__(self, eps=1.0, max_iter=20):
        super().__init__()
        self.eps = eps
        self.max_iter = max_iter

    def forward(self, pred_logits, target_probs, cost_matrix):
        log_mu = torch.nn.functional.log_softmax(pred_logits, dim=-1)
        log_nu = torch.log(target_probs + 1e-9)
        C = cost_matrix
        log_K = -C / self.eps
        ...
        for _ in range(self.max_iter):
            term_f = log_K + g.unsqueeze(1)
            f = log_mu - torch.logsumexp(term_f, dim=2)
            term_g = log_K.transpose(1, 2) + f.unsqueeze(1)
            g = log_nu - torch.logsumexp(term_g, dim=2)

        log_Gamma = f.unsqueeze(2) + g.unsqueeze(1) + log_K
        Gamma = torch.exp(log_Gamma)
        loss = torch.sum(Gamma * C, dim=(1, 2))
        return loss.mean()
...
class Simulator:
    def __init__(self):
        ...
        self.ot_utils = DistributionalOTUtils(self.device, min_return_bb=-200, max_return_bb=500, n_atoms=101)
        self.sinkhorn_loss = LogSinkhornLoss(eps=1.0, max_iter=20)
    ...
    def rl(self):
        ...
        print(f"Starting Training with Log-Sinkhorn Distributional Loss...")
                ...
                if training_indices:
                    ...
                    for b_idx, t in training_indices:
                        ...
                        if len(valid_means) > 0:
                            ...
                            exp_vals = np.exp((means - max_val) / (0.5 * hand_bb))
                        ...
                        all_dists_flat.append(item_samples)
                        all_bbs_flat.extend([hand_bb] * 5)
                    ...
                    if all_pred_logits:
                        ...
                        flat_rewards = []
                        for item in all_dists_flat:
                            for action_samples in item:
                                flat_rewards.append(action_samples)

                        bb_tensor = torch.tensor(all_bbs_flat, device=self.device, dtype=torch.float32)
                        all_histograms = self.ot_utils.to_categorical(flat_rewards, bb_tensor)

                        dist_tensor = all_histograms.view(-1, 5, self.ot_utils.n_atoms)
                        cost_matrices = self.ot_utils.compute_pairwise_wasserstein(dist_tensor)

                        if self.global_updates % 100 == 0:
                            print(f"\n[Debug] Update {self.global_updates}")
                            print(f"Max Cost: {cost_matrices.max().item():.2f}")
                            print(f"Target Probs (Ex 0): {batch_targets[0].tolist()}")

                        loss = self.sinkhorn_loss(batch_preds, batch_targets, cost_matrices)
